...
function newton(z, x, y, x0; tol=1e-4, maxit=1000, λ_reg=0.0)
    koraki = [x0]
    X = x0
    k = length(x0) ÷ 2
...
res_newton = newton(z, x, y, x0, λ_reg=0.0)
...
# Newton final result
scatter!(ures_newton, vres_newton, label="Newton Result", color=:blue, markersize=8, marker=:diamond)

for (idx, step) in enumerate(res_newton.koraki)
    if idx % 20 == 1  # Change 3 to something else to control frequency
        u_step = step[1:4]
        v_step = step[5:8]
